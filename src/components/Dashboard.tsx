...
import {
  ...
  DocumentType,
  ProductivityPage,
  ActivityLog, // Import ActivityLog
  Customer, // Import Customer
} from '../types';
import ActivityLogComponent from './ActivityLog'; // Import our new component
...
}> = ({ documents, totalRevenue, pendingAmount }) => {
    ...
    <div className="bg-white dark:bg-zinc-900 p-6 rounded-xl shadow-sm h-full relative flex flex-col">
      ...
      <div className="w-full h-[220px] flex-grow" onMouseLeave={() => setTooltip(null)}>
        <svg
          width="100%"
          height="100%"
          viewBox={`0 0 ${chartData.labels.length * (barWidth + barMargin)} ${chartHeight + 30}`}
        >
          {[...Array(5)].map((_, i) => (
            <line
              key={i}
      ...
      <div className="space-y-3">
        {actions.map((action) => (
          <button
            ...
            className="w-full flex items-center gap-3 p-3 rounded-lg bg-slate-100 dark:bg-zinc-800 hover:bg-slate-200 dark:hover:bg-zinc-700 transition-colors"
          >
...
}> = ({ documents, editDocument, overdueAmount }) => {
  const overdue = useMemo(
    () => documents.filter((doc) => doc.status === DocumentStatus.Overdue),
    [documents]
  );
    ...
    <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-500/30 p-6 rounded-xl shadow-sm h-full flex flex-col">
      <div className="flex-shrink-0">
        <div className="flex justify-between items-start">
          ...
          <span className="px-2 py-1 text-xs font-bold text-red-800 bg-red-200 dark:bg-red-800 dark:text-red-100 rounded-full">
            {overdue.length}
          </span>
        </div>
        <p className="text-xs text-red-600 dark:text-red-300 mt-2">Total Amount Overdue</p>
        <p className="text-3xl font-bold text-red-700 dark:text-red-200 mt-1 mb-4">
        ...
        {overdue.map((doc) => (
          <li key={doc.id}>
            <button
              ...
              className="w-full text-left p-2 rounded-lg hover:bg-red-100 dark:hover:bg-red-900/30 transition-colors"
            >
...
const RecentPagesPanel: React.FC<{ pages: ProductivityPage[] }> = ({ pages }) => {
  ...
  return (
    <div className="bg-white dark:bg-zinc-900 p-6 rounded-xl shadow-sm flex flex-col h-full">
      <Link
        to="/productivity-hub"
        className="text-xl font-semibold mb-4 text-slate-800 dark:text-zinc-100 hover:text-primary-600 dark:hover:text-primary-400 transition-colors"
      >
        Recent Pages &rarr;
      </Link>
            ...
            {recentPages.map((page) => (
              <li key={page.id}>
                <Link
                  ...
                  className="block p-3 rounded-lg hover:bg-slate-100 dark:hover:bg-zinc-800 transition-colors"
                >
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Here is the new Activity Log component */}
        <div className="lg:col-span-2 bg-white dark:bg-zinc-900 p-6 rounded-xl shadow-sm">
          <h2 className="text-xl font-semibold mb-4 text-slate-800 dark:text-zinc-100">
            Recent Activity
          </h2>
          <ActivityLogComponent
            activityLogs={recentActivity} // Pass only recent logs
            customers={customers}
            addActivityLog={addActivityLog}
            showCustomerName={true} // This will show "for {customer.name}"
          />
        </div>
      </div>
    </div>
